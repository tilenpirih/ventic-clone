<template>
  <v-layout justify-space-between class="pa-3 pl-3">
    <v-layout justify-end align-center>
      <Slider v-if="selectedLayout === 0" class="mr-3" />
      <v-menu
        v-model="layoutMenu"
        bottom
        left
...
export default {
  ...
  computed: {
    showCardWidth: {
      get() {
        return this.$store.state.SelectedMedia.showCardWidth
      },
      set(value) {
        this.$store.commit('SelectedMedia/SET_SHOW_CARD_WIDTH', value)
      },
    },
    showLayoutType() {
  ...
  mounted() {
    this.selectedLayout = this.layouts.findIndex(
      layout => layout.componentName === this.showLayoutType
    )
  },

  methods: {
    changeLayoutType(layout) {
      this.$store.commit('SelectedMedia/SET_SHOW_LAYOUT_TYPE', layout)
    },
    goBack() {
      this.$router.go(-1)
    },
