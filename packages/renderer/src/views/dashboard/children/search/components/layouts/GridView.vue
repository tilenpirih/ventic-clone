    ...
    <template #default="{ item }">
      <v-layout v-if="item !== true" class="my-1">
        <v-flex v-for="(media, index) in item" :key="index">
          <MovieGridCard v-if="media.media_type === 'movie'" :movie="media" />
          <ShowGridCard v-if="media.media_type === 'tv'" :show="media" />
        </v-flex>
        <v-flex
...
import MovieGridCard from '@/views/dashboard/components/layout_components/movie/GridCard.vue'
import ShowGridCard from '@/views/dashboard/components/layout_components/show/GridCard.vue'
import InfiniteLoading from 'vue-infinite-loading'

export default {
  name: 'GridView',
  components: { MovieGridCard, ShowGridCard, InfiniteLoading },
  mixins: [sleep],
  data: () => ({
    itemHeight: 0,
    mediasPerRow: null,
  ...
  methods: {
    ...
    onResize() {
      ...
      this.itemHeight = (((container - 12 - numOfMedias * 12) / numOfMedias) * 3) / 2 + 12
