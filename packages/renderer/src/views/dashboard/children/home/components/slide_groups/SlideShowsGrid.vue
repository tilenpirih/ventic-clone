<template>
  <div>
    <v-layout justify-space-between class="pl-3 title">
      <span class="text--text">Popular shows</span>
      <div class="mr-3">
...
import Grid from '@/views/dashboard/components/layout_components/show/GridCard.vue'
import DetailGrid from '@/views/dashboard/components/layout_components/show/DetailGridCard.vue'
...
export default {
  ...
  computed: {
    shows() {
      try {
        const shows = JSON.parse(JSON.stringify(this.$store.state.Shows.shows.popular.all.results))
        shows.push({ id: true })
        return shows
      } catch (e) {
        return [{ id: true }]
  ...
  beforeDestroy() {
    this.$store.commit('Home/SET_POPULAR_SHOWS_SCROLL_POSITION', this.scrollPosition)
  },
  mounted() {
    this.$nextTick(async () => {
      this.showInfiniteLoading = true
      await this.sleep(1)
      this.$refs.scroller.$el.scrollLeft = this.$store.state.Home.popularShowsScrollPosition
    })
  },
  methods: {
