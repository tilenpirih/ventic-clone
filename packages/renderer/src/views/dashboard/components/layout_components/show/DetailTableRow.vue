<template>
  <v-layout
    class="pa-3 mx-3 pointer"
    :class="[show.index % 2 ? 'back1' : 'back2']"
    @click.stop="openShow()"
  >
    <v-img
      :aspect-ratio="2 / 3"
    ...
    <div>
      <v-layout column>
        <div class="title text--text noSelect">{{ show.name }}</div>
        <div class="text3--text noSelect">
          <b>{{ formatReleaseDate(show.first_air_date) }}</b>
        </div>
        <div class="overview text--text noSelect">{{ show.overview }}</div>
      </v-layout>
    </div>
    <v-flex class="ml-2">
